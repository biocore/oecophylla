...
def read_benchmark(fp, pattern = '^.*benchmarks/(.+?)/(.+?)\.sample=\[(.+?)\](\.abund_sample=\[(.+?)\])?\.txt$'):
    ...
    if m:
        ...
        benchmarks['abund_sample'] = m.group(5)
...
def load_benchmarks(benchmark_fps, multiqc, cluster_json):
    ...
    benchmark_df['sample_seqs'] = multiqc.loc[benchmark_df['sample'],'FastQC_total_sequences'].values

    # remove non-numeric and redundant h:m:s
    benchmark_df.drop('h:m:s', axis=1, inplace=True)
    
    # add abund sample seq # from multiqc df
    benchmark_df['abund_seqs'] = np.nan
    a = benchmark_df['abund_sample'].dropna()
    benchmark_df.loc[a.index,'abund_seqs'] =  multiqc.loc[a,'FastQC_total_sequences'].values
    ...
    benchmark_tidy = pd.melt(benchmark_df,
               id_vars=['benchmark_file', 'rule', 'module', 'sample', 'abund_sample', 'sample_seqs', 'abund_seqs', 'n_cpus', 'mem'])
